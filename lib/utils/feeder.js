...
export async function triggerFeed({ type, user, db, feederRef, now }) {
  ...
  // CRITICAL: Update lastFeedTime and lastFeed in parallel (they don't depend on each other)
  // This prevents Arduino from overwriting with invalid millis()
  // Then set status = 1 after both complete
  await Promise.all([
    feederRef.child('lastFeedTime').set(timestampMs),
  ...
  setTimeout(async () => {
    try {
      ...
      console.error('[FEEDER] Error updating history:', err);
    }
  }, 0);
